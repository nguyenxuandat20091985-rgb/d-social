...
export default async function handler(req, res) {
  ...
  try {
    ...
    const payment = await payos.createPaymentLink({
      orderCode,
      amount,
      description: type === 'vip_purchase' ? `D VIP ${orderCode}` : `D nap ${orderCode}`,
      returnUrl: `${origin}/?payment=success&order=${orderCode}`,
      cancelUrl: `${origin}/?payment=cancel&order=${orderCode}`,
    })
